...
sigm_d1d2sero_default_theta = sigm_d1d2sero_Theta(
    ...
    Rd1=1., Rd2=1., Rs=1., Sd1=-5.0, Sd2=-5.0, Ss=-20.0, Zd1=0.7, Zd2=0.7, Zs=.35, tau_Md1=1000, tau_Md2=1000, tau_Ms=1000,  
    we=1e-2, wi=1e-2, wd=1e-2, ws=1e-2, sigma_V=0., sigma_u=0.,
)
...
def sigm_d1d2sero_stay_positive(y, _):
    """Keep certain state variables non-negative for sigm_d1d2sero model."""
    y = y.at[0].set(jp.where(y[0] < 0, 0, y[0]))  # r
    y = y.at[5].set(jp.where(y[5] < 0, 0, y[5]))  # Dp
    y = y.at[6].set(jp.where(y[6] < 0, 0, y[6]))  # Se
